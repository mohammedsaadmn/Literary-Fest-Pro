...
import { User, Trophy, MapPin, Coins, Users, Sparkles, AlertCircle, Shield, Award, Hash, Zap, CheckCircle2 } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-3 sm:p-6 pb-12 selection:bg-amber-400 selection:text-slate-950">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {currentStudent ? (
          <div className="bg-slate-900/95 border border-slate-800 rounded-3xl p-4 sm:p-6 shadow-2xl backdrop-blur-xl flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4 w-full sm:w-auto">
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center border-2 border-amber-400 shadow-xl shrink-0">
                <User size={36} color="white" />
              </div>
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="px-2.5 py-0.5 bg-amber-400/10 text-amber-400 border border-amber-400/20 text-[10px] sm:text-xs font-black rounded-full uppercase tracking-wider">
                    🏆 Round {auctionRound}
                  </span>
                  <span className="px-2.5 py-0.5 bg-blue-500/10 text-blue-400 border border-blue-500/20 text-[10px] sm:text-xs font-extrabold rounded-full uppercase tracking-wider">
                ...
                <div className="flex items-center gap-3 text-xs text-slate-300 font-semibold mt-0.5">
                  <span className="flex items-center gap-1 text-amber-300">
                    <Trophy size={13} /> {currentStudent.category}
                  </span>
                  <span className="flex items-center gap-1 text-blue-400">
                    <MapPin size={13} /> {currentStudent.place}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between w-full sm:w-auto gap-4 bg-slate-950/80 px-4 py-3 rounded-2xl border border-slate-800">
              <div>
              ...
              <div className="text-right">
                <p className="text-[10px] text-slate-400 uppercase font-extrabold tracking-widest">Timer</p>
                <p className={`text-xl sm:text-2xl font-black ${timer <= 5 ? "text-red-500 animate-pulse" : "text-green-400"}`}>
                  {timer}s
                </p>
              </div>
            </div>
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 text-center text-slate-400 font-bold backdrop-blur-xl">
            Waiting for next student to enter auction stage...
          </div>
        )}

        {/* Main Captain Card */}
        <div
          className="rounded-3xl p-6 sm:p-8 text-white shadow-2xl border border-white/10 relative overflow-hidden backdrop-blur-xl space-y-6"
          style={{
        ...
        >
          ...
          <div className="flex justify-between items-start border-b border-white/20 pb-5">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-white/80 text-xs font-semibold">Captain Mobile Console</span>
          ...
          {squadFull && (
            <div className="bg-red-500/20 border border-red-500/40 text-red-200 p-3.5 rounded-2xl font-bold text-center text-xs sm:text-sm">
              🚫 Squad Full ({maxStudents}/{maxStudents})
            </div>
          )}
          {categoryFull && (
            <div className="bg-red-500/20 border border-red-500/40 text-red-200 p-3.5 rounded-2xl font-bold text-center text-xs sm:text-sm">
          ...
          {maximumAllowedBid <= auction.currentBid && !squadFull && (
            <div className="bg-red-500/20 border border-red-500/40 text-red-200 p-3.5 rounded-2xl font-bold text-center text-xs sm:text-sm">
              🚫 You must reserve enough budget to complete your squad.
            </div>
          )}

          {/* Mobile-First Rapid Tap Bidding Controls */}
          <div className="space-y-3 pt-2">
            <p className="text-xs uppercase font-extrabold tracking-wider text-white/80 flex items-center gap-1.5">
              ⚡ Place Bid Increment (+500, +1000, +2000, +5000)
            </p>
              ...
              {[500, 1000, 2000, 5000].map((increment) => {
                ...
                return (
                  <button
                    ...
                    className={`py-4 px-3 rounded-2xl font-black text-base sm:text-lg transition-all shadow-xl active:scale-95 flex items-center justify-center ${
                      canAfford
                        ? "bg-white text-slate-950 hover:bg-amber-300 hover:shadow-amber-400/30 cursor-pointer"
        ...
        <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 sm:p-7 shadow-2xl backdrop-blur-xl space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="bg-slate-950/80 border border-slate-800 px-3 py-1.5 rounded-xl text-center">
                <span className="text-[10px] text-slate-400 font-extrabold uppercase block">Squad Count</span>
          ...
          {squadList.length === 0 ? (
            <div className="bg-slate-950/50 border border-slate-800/80 rounded-3xl p-10 text-center space-y-3">
              ...
              <h4 className="text-lg font-black text-slate-300">No Purchased Players Yet</h4>
              <p className="text-xs text-slate-500 max-w-md mx-auto font-medium">
                Winning bids in live auction rounds will automatically add players to your team roster here in real time.
              </p>
            </div>
          ) : (
